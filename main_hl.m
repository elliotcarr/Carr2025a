...
% Test problem
tau = 0.001; problem = '1'; sigma_vals = [0,0.05];  te = 0.4; Ms = 1001;
...
maxFnEvals = 10000; % parameter for numerical LaplaMce inversion
method = 'cme'; % method for numerical Laplace inversion
qtL = @(s) (Qinf*(tau/(s + 1/beta) + (beta - tau)/(beta*(s + 1/beta)^2)))/beta^2;
...
%% Legend (for Figures 3 and C.2 of paper)
if plot_legend
    figure(5)
    set(gcf,'Color','w')
    scsize = get(gcf,'Position');
